PlaytypeState = Class{__includes = BaseState}

local highlighted = 1

function PlaytypeState:update(dt)
    ...
    if love.keyboard.wasPressed("enter") or love.keyboard.wasPressed("return") then
        if highlighted == 2 then
            gStateMachine:change("pong-two-player-start")
        elseif highlighted == 1 then
            gStateMachine:change("pong-one-player-start")
        end
    end
...
function PlaytypeState:render()
    love.graphics.setFont(gFonts["large"])
    love.graphics.setColor(2/255, 215/255, 242/255, 1)
    love.graphics.printf("PONG UPGRADE", 0, V_HEIGHT/3, V_WIDTH, "center")
