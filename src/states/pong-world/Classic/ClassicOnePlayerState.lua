ClassicOnePlayerState = Class{__includes = BaseState}

paddle_Speed = 200

function ClassicOnePlayerState:init()
...
function ClassicOnePlayerState:update(dt)
	...
	if ball.x >V_WIDTH then
		...
		if player1Score == 7 then
			WinningPlayer = 1
            gameState = "done"
            gSounds["girls-cheers"]:play() -- remember to change the music to disappointment
		else
			gameState = "serve"
		end
		ball:reset()
...
function ClassicOnePlayerState:render()
	push:apply("start")
