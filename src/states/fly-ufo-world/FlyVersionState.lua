FlyVersionState = Class{__includes = BaseState}

local highlighted = 1

function FlyVersionState:update(dt)
    ...
    if love.keyboard.wasPressed("enter") or love.keyboard.wasPressed("return") then
        if highlighted == 2 then
            gStateMachine:change("fly-start")
        elseif highlighted == 1 then
            gStateMachine:change("flappy-start")
        end
    end
...
function FlyVersionState:render()
    love.graphics.setFont(gFonts["large"])
    love.graphics.setColor(2/255, 215/255, 242/255, 1)
    love.graphics.printf("FLY UFO", 0, V_HEIGHT/3, V_WIDTH, "center")
